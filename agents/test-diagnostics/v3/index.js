...
async function gradeAnswers(state) {
    ...
    const gradingLength = Math.min(userAnswers.length, testQuestions.length);
    ...
    for (let i = 0; i < gradingLength; i++) {
        ...
        console.log(`[Graph] Q${i}: User:${userAnswer}, Correct:${correctAnswer} -> ${isCorrect ? 'Correct' : 'INCORRECT'}`);
    }

    console.log(`[Graph] Grading complete. Correct: ${gradedResults.filter(r => r.isCorrect).length}/${gradingLength}`);
    return { gradedResults };
}
